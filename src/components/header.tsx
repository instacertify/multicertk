    ...
    <header className="sticky top-0 z-40 border-b border-line/80 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          <Link href="/search" className="hidden text-sm font-semibold text-navy hover:text-gold-600 sm:inline">
            {t("search")}
          </Link>
          <LocaleSwitcher />
          <Link
            href="/contact"
            className="hidden rounded-full bg-gold px-4 py-2 text-sm font-semibold text-navy hover:bg-gold-600 md:inline"
          >
            {cta("quote")}
          </Link>
          <button
            type="button"
            className="rounded-md border border-line px-3 py-1.5 text-sm font-semibold lg:hidden"
